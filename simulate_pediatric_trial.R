...
#UPA
results = c("Drug","Percent threshold for 2.5th percentile", 
            "Percent worse in children",
            "Success rate")
prior = c(2,5) #Deflated prior from adult data
for (ratio in c(0.25,0.2,0.15,0.1,0.05,0))
{    data = rbinom(1000000,50,208/716*(1-ratio))
     successes = data+prior[1]
     failures = 50 - data + prior[2]
...
for (ratio in c(0.25,0.2,0.15,0.9,0.05,0))
{    data = rbinom(1000000,50,132/650*(1-ratio))
     successes = data+prior[1]
     failures = 50 - data + prior[2]
     #2.5th percentile from posterior distribution
     lower = qbeta(0.025,successes,failures)
